...
/*   Updated: 2020/12/20 11:51:32 by dcapers          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	*token = (*token)->next;
	if (!(p->name = ft_strsub((*token)->content, 0, ft_strlen((*token)->content))))
...
{
	*token = (*token)->next;
	if (!(p->comment = ft_strsub((*token)->content, 0, ft_strlen((*token)->content))))
		kill_exe(ERR_STR_INIT);
