...
/*   Updated: 2021/02/12 09:48:33 by student          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			parse_command(t_parser *p, char *row, t_token *t)
{
	...
	if (ft_strcmp(t->content, NAME_CMD_STRING))
		p->fname = 1;
	if (ft_strcmp(t->content, COMMENT_CMD_STRING))
		p->fcomment = 1;
	if (ft_strcmp(t->content, NAME_CMD_STRING) &&
...
void			parse_asm(t_parser *parser)
{
	...
	if (parser->fcomment == 0 || parser->fname == 0)
	{
		ft_putstr("no name or comment\n");
		exit(1);
	}
